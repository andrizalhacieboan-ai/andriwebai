...
// Instruksi dibuat lebih ringkas dan natural agar selalu lolos filter Gemini Web
const DEFAULT_INSTRUCTION = 'Anda adalah asisten AI yang cerdas. Selalu jawab menggunakan bahasa yang sama dengan pertanyaan pengguna. Jika diminta menulis atau memperbaiki kode, WAJIB gunakan blok kode Markdown standar (contoh: ```javascript atau ```python) agar kode mudah dibaca.'
...
const parseGeminiFrames = (data) => {
    ...
    for (let i = frames.length - 1; i >= 0; i -= 1) {
        ...
        // PERBAIKAN UTAMA: Ekstraksi teks yang lebih robust untuk menangani kode
        let text = ''
        ...
        if (typeof textNode === 'string') {
            ...
            // Jika berupa array (mix teks biasa & blok kode), gabungkan semuanya
            text = textNode.map(item => {
                if (typeof item === 'string') return item
                if (typeof item === 'object' && item !== null) {
                    // Tangkap object kode dari Gemini
