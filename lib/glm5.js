...
export default async function glm5(prompt) {
  ...
  if (contentType.includes('application/json') && !rawText.includes('data:')) {
    ...
    throw new Error('Format JSON tidak dikenali: ' + rawText.substring(0, 200));
  }
  ...
  for (const line of lines) {
    ...
    try {
      ...
      if (!sampleParsed) sampleParsed = parsed;

      if (parsed.error) {
        throw new Error(parsed.error.message || 'Error dari server');
      }
      ...
      if (choice) {
        ...
        else if (typeof delta.text === 'string' && delta.text) fullText += delta.text;
      ...
      if (typeof parsed.response === 'string' && parsed.response) fullText += parsed.response;
      if (typeof parsed.content === 'string' && parsed.content) fullText += parsed.content;
      if (typeof parsed.result === 'string' && parsed.result) fullText += parsed.result;
      if (typeof parsed.answer === 'string' && parsed.answer) fullText += parsed.answer;
      if (typeof parsed.output === 'string' && parsed.output) fullText += parsed.output;
    } catch (e) {
  ...
  if (!fullText.trim()) {
    const sample = sampleParsed ? JSON.stringify(sampleParsed).substring(0, 300) : 'Tidak ada JSON valid';
    const rawSample = rawText.replace(/\n/g, ' ').substring(0, 300);
    throw new Error(`Tidak ada teks. SampleJSON: ${sample}. Raw: ${rawSample}`);
  }
