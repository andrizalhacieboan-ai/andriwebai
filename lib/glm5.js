...
async function getExistingChatId(authHeaders) {
  try {
    ...
    if (e.response && e.response.status === 401) {
      throw new Error('Token sudah kadaluarsa atau tidak valid. Dapatkan token baru dari browser.');
    }
  }
  return null;
}
...
export default async function glm5(prompt) {
  ...
  let streamResponse;
  try {
    streamResponse = await axios.post(
    ...
    streamResponse.data.on('data', (chunk) => {
      ...
      for (const line of lines) {
        ...
        try {
          ...
          if (parsed.error) {
            reject(new Error(parsed.error.message || 'Error dari server GLM-5'));
            return;
          }
