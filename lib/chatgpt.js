...
export class GptService {
  static async process(input, options = {}) {
    try {
      ...
      // Struktur payload dibuat sespesifik mungkin tanpa null yang dapat merusak parser backend
      ...
      if (cookies) {
        mainHeaders['Cookie'] = cookies;
        ingestHeaders['Cookie'] = cookies;
      }
      ...
      /* 3. Menjalankan request utama ke API Chat (Dengan Stringify Manual) */
      // Kita stringify manual agar axios tidak merusak struktur payload
      const response = await axios.post(apiChatUrl, JSON.stringify(requestPayload), { 
        headers: mainHeaders 
      });
