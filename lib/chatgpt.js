...
export class ChatGPT {
  constructor(cfg = {}) {
    ...
    this.cookies = null; // Tambahkan penyimpanan cookie
  }

  // Fungsi baru: Inisialisasi sesi untuk mengambil Cookie dari ChatGPT
  async initSession() {
    if (this.cookies) return;
    try {
      const res = await _fetch(this.baseUrl, {
      ...
      // Ekstrak Set-Cookie headers
      const rawHeaders = res.headers.raw?.()['set-cookie'] || [];
      if (rawHeaders.length > 0) {
        this.cookies = rawHeaders.map(c => c.split(';')[0]).join('; ');
      } else {
      ...
      console.error("Gagal mengambil cookie sesi:", e.message);
    }
  }
  ...
  async send(message, opts = {}) {
    ...
    // Wajib jalankan init session sebelum request API
...
Anda adalah seorang pakar pemrograman dan analis kode senior.
Tugas Anda adalah menganalisis, memperbaiki bug, dan menulis kode (JavaScript, HTML, CSS, Python, PHP, dll) dengan sangat profesional.
...
3. Jangan pernah menghilangkan tanda backticks (\`\`\`) saat mengirim kode.
4. Jika user tidak mengirim kode, jawab pertanyaan mereka dengan normal.`;
