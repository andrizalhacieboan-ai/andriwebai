...
// Gunakan globalThis.fetch (bawaan Node 18+ / Vercel), atau fallback ke undici
let _fetch = globalThis.fetch;
if (!_fetch) {
  // Dynamic import untuk environment yang belum punya fetch native
...
export class GptService {
  static async process(input, options = {}) {
    try {
      ...
      // Payload dikembalikan persis 100% seperti kode asli
      ...
      const response = await _fetch(apiChatUrl, {
        method: "POST",
        headers: mainHeaders,
        body: JSON.stringify(requestPayload) // Fetch mengharuskan stringify manual
      });
